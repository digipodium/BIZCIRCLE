        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <p className="text-[11px] text-slate-400 uppercase tracking-wider font-semibold mb-2">About / Bio</p>
              {editMode ? (
                <textarea
                  value={form?.bio || ""}
                  onChange={(e) => setForm((f) => ({ ...f, bio: e.target.value }))}
                  rows={3}
                  className="w-full text-sm text-slate-800 font-medium bg-white border border-blue-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-400 leading-relaxed"
                  placeholder="Tell us a bit about yourself..."
                />
              ) : (
                <p className="text-sm text-slate-700 leading-relaxed">
                  {user?.bio || <span className="text-slate-400 italic">No bio written yet. Click edit to add one.</span>}
                </p>
              )}
            </div>
          </div>
